...
class sub_object(object):
    ...
    def __init__(self, rank_to_use, **options):
        ...
        generation_type = options['generation_type'] \
                                                ...
                                                else randomly.pop(["default",
                                                                "alternative"])

        figures = [str(i+1) for i in range(9)]

        ranks_scale = options['ranks_scale'] if 'ranks_scale' in options \
                                             else copy.copy(DEFAULT_RANKS_SCALE)
        ...
        if not "direct" in options:
            if 'rank_matches_invisible_zero' in options \
                ...
                and options['rank_matches_invisible_zero'] != "False":
            #___
        ...
        if generation_type == "default":
            ...
            if not rank_matches_invisible_zero:
                if not 'numberof' in options:
                    ...
                    for r in range(highest_start_rank - lowest_start_rank + 1)]
