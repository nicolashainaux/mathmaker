...
def __main__():
    ...
    creation_query = '''CREATE TABLE pythagorean_triples
                        (id INTEGER PRIMARY KEY, leg0 INTEGER, leg1 INTEGER,
                         ...
                         calculate_leg INTEGER, suits_for_decimals INTEGER,
                         exactness TEXT, drawDate INTEGER)'''
    db_creation_queries.append(creation_query)
    db.execute(creation_query)

    db_rows = [(leg0, leg1, hyp, 1, 1,
                0 if all([not leg0 % 10, not leg1 % 10, not hyp % 10]) else 1,
                'exact', 0) for (leg0, leg1, hyp) in ALL_TRIPLES_5_200]
    db.executemany("INSERT "
                   "INTO pythagorean_triples(leg0, leg1, hyp, calculate_hyp, "
                   "calculate_leg, suits_for_decimals, exactness, drawDate) "
                   "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
                   db_rows)

    DATA = NO_TRIPLES_LEGS_1_200 + NO_TRIPLES_HYPS_1_200
    db_rows = [(leg0, leg1, hyp, 1 if hyp == 0 else 0, 1 if leg0 == 0 else 0,
                0 if all([not leg0 % 10, not leg1 % 10, not hyp % 10]) else 1,
                'approx', 0) for (leg0, leg1, hyp) in DATA]
    db.executemany("INSERT "
                   "INTO pythagorean_triples(leg0, leg1, hyp, calculate_hyp, "
                   "calculate_leg, suits_for_decimals, exactness, drawDate) "
                   "VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
